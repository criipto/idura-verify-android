...
/**
 * A host implementing [LifecycleOwner] whose lifecycle never leaves INITIALIZED, standing in for a
 * consumer that exposes one and never drives it.
 *
 * Deliberately not a `LifecycleRegistry`. With `lifecycle-process` on the classpath — it is here, and
 * in any app that depends on it — androidx's own `ReportFragment` drives the registry of every
 * activity that exposes one, so a registry cannot be held at INITIALIZED to test against. An app
 * without that dependency gets no such help, and a host that forgets to drive its own registry is
 * what this stands in for.
 */
class TestUndrivenLifecycleHostActivity :
  Activity(),
  LifecycleOwner {
...
class ActivityResultRegistrationTest {
  ...
  fun aLifecycleThatIsNeverDrivenIsNamedAsTheProblem() {
    ActivityScenario.launch(TestUndrivenLifecycleHostActivity::class.java).use { scenario ->
      lateinit var iduraVerify: IduraVerify

      scenario.onActivity { activity ->
        // Guards the premise: a host whose lifecycle something else drives would take this test
        // through a passing login instead, and it would stop covering anything.
  ...
  /**
   * A plain-activity host forwards every result it gets, so a result belonging to the host itself
   * must come straight back out. Which codes are the SDK's own is between it and its registry, and
   * covered in [HostActivityResultRegistryTest].
   */
  @Test
  fun aPlainActivityHostDoesNotClaimAResultItNeverLaunched() {
    ActivityScenario.launch(TestPlainHostActivity::class.java).use { scenario ->
      scenario.onActivity { activity ->
  ...
  fun aComponentActivityHostDoesNotRouteResultsItself() {
    ActivityScenario.launch(TestHostActivity::class.java).use { scenario ->
      scenario.onActivity { activity ->
        ...
        // Claims nothing at all, whatever the code — here one from the range the registry hands
        // out, so a launcher of ours could plausibly be sitting on it.
        assertFalse(
          iduraVerify.handleActivityResult(0x10000, Activity.RESULT_CANCELED, null),
        )
      }
    }
  }
  ...
  /**
   * The duplicate-instance claim is per activity, whichever kind of host it is — the same typed
   * rejection as [identicallyConfiguredInstancesAreRejected], so a consumer's handling of it does
